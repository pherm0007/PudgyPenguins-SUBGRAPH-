...
export class CreatePenguin extends Entity {
  constructor(id: Bytes) {
    super();
    this.set("id", Value.fromBytes(id));
  }
  ...
  save(): void {
    let id = this.get("id");
    assert(id != null, "Cannot save CreatePenguin entity without an ID");
    if (id) {
      assert(
        id.kind == ValueKind.BYTES,
        `Entities of type CreatePenguin must have an ID of type Bytes but the id '${id.displayData()}' is of type ${id.displayKind()}`,
      );
      store.set("CreatePenguin", id.toBytes().toHexString(), this);
    }
  }

  static loadInBlock(id: Bytes): CreatePenguin | null {
    return changetype<CreatePenguin | null>(
      store.get_in_block("CreatePenguin", id.toHexString()),
    );
  }

  static load(id: Bytes): CreatePenguin | null {
    return changetype<CreatePenguin | null>(
      store.get("CreatePenguin", id.toHexString()),
    );
  }
  ...
  get PudgyPenguins_id(): BigInt {
    let value = this.get("PudgyPenguins_id");
    if (!value || value.kind == ValueKind.NULL) {
      throw new Error("Cannot return null for a required field.");
    } else {
      return value.toBigInt();
  ...
  set PudgyPenguins_id(value: BigInt) {
    this.set("PudgyPenguins_id", Value.fromBigInt(value));
  }
